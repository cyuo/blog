...
![VPS性能测试图](/img/photo_2025-08-07_03-50-51.webp)
...
```bash
nano /etc/sysctl.d/00-opti-vt.conf
```
...
```bash
sysctl -p /etc/sysctl.d/00-opti-vt.conf
```
...
**不要盲目复制粘贴**到所有服务器上。对于一个内存很小（如 1GB）的 VPS，过大的缓冲区可能会消耗过多内存，反而导致系统不稳定。请根据实际服务器配置和用途进行适当调整。